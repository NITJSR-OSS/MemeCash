...
const ipfs=new ipfsClint({host:'ipfs.infura.io', port:5001,protocol:'https'});
 ...
 function Upload({closeModal}) {
   ...
   const d= async(e)=>{
    e.preventDefault();
     console.log('hola');
      ...
      await memeFactoryContract.methods.createMeme(responce.path,responce.path).send({from:account});
      closeModal();
   ...
   useEffect(() => {
    ...
    return (
      <div>
        <img src={file??'https://via.placeholder.com/150/000000/FFFFFF/?text=meme cash'} alt='jks' id="uploaded-img" />
        <form onSubmit={handelSubmit} >
          ...
          <button className="new-button"  disabled={desableUpload} onClick={d} >Upload</button>
          {/* <input  type='submit'   /> */}
          </div>
        </form>
